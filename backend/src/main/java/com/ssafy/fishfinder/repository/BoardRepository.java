...
//    @Query("select p from Post p where p.postType in :postType and p.createdAt < :createdAt order by p.createdAt desc")
    @Query(nativeQuery = true, value = "select * from post where post_type in :#{T(com.ssafy.fishfinder.util.PostTypeUtil).getPostTypeList(#postType)} and created_at < :createdAt order by created_at desc limit :limit")
...
//    @Query("select p from Post p where p.postType in :postType and (select count(l) from Likes l where l.post = p) < :likeCount order by (select count(l) from Likes l where l.post = p) desc")
    @Query(nativeQuery = true, value = "select * from post where post_type in :#{T(com.ssafy.fishfinder.util.PostTypeUtil).getPostTypeList(#postType)} and (select count(*) from likes where post_id = post.post_id) < :likeCount order by (select count(*) from likes where post_id = post.post_id) desc, created_at desc limit :limit")
